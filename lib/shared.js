...
(function (root) {
  ...
  /** 生成图片原色补偿所需的 SVG feColorMatrix 片段。 */
  function buildInverseSVGMarkup(settings) {
    var p = filterParts(settings);
    return '<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" ' +
      'style="position:absolute;top:0;left:0;overflow:hidden" aria-hidden="true" focusable="false">' +
      '<defs><filter id="' + EyeProtector.INVERSE_FILTER_ID + '" color-interpolation-filters="sRGB">' +
      '<feColorMatrix type="matrix" values="' + p.inverseMatrixValues.join(' ') + '"/>' +
      '</filter></defs></svg>';
  }
  ...
  var EyeProtector = {
    ...
    buildInverseSVGMarkup: buildInverseSVGMarkup,
